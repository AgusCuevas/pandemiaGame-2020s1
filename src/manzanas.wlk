...
class Manzana {
	...
	method cantidadDePersonasInfectadasYNoAisladas(){
		return self.personasInfectadasYNoAisladas().count()
	}
	...
	method aislarATodosLosInfectadosSinAislar(){
		self.personasInfectadasYNoAisladas().forEach({ per => per.aislar()})
	}
